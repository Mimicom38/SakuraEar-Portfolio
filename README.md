...
#これはサンプルです。うまくできていないので、提出用ではありません。このプロジェクトのコードの大部分は、AIやGoogle検索でかきあげたものです。
# 概要（Overview）
...
耳の切り方が浅い、角度が微妙、判断が困難な画像も一部含まれています。 これは現実のTNR現場でも見られる課題であり、学習モデルの汎用性と柔軟性向上のために採用しています。 ただし、画像枚数が少なく精度に限界がありますので、結果は目安となります。
...
TNR活動の目印であるさくら耳を判別するモデルを通して保護猫や地域猫の識別が少しでも効率化され、保護団体の支援に繋がれば幸いです。
このプロジェクトは小さな一歩ですが、動物や福祉に貢献できるエンジニアを目指します。
