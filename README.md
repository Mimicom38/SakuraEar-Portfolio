...
制作時に最も時間を費やしたのは、手作業による画像加工です。初めは200枚ずつ計400枚の画像を集めましたが作業時に誤って消去したため、初めから拾い直し、(少々枚数が足りませんでしたが)386枚の画像を集めました。左右反転やcanvaでの切り抜きで背景除去して試しましたが精度が一向にあがらず、コードをSimpleCNNからResNet18に、transformの画像加工の過程を変更して試しました。しかし、ChatGPTによると、背景除去はこの細かな画像分類には不向きだとのことで、三度目の画像収集をしました。今度は顔まわり、耳を中心に背景を除去せず一枚一枚手作業で切り抜き、sakura/notsakura524枚(各262枚)のデータセットを作りました。分類はsplitで自動分類し、左右反転もコードで行いましたが、それでも時間がかかりました。

*モデル構築　おわりに*
本プロジェクトのコーディングにあたっては、機械学習の理解度が初歩的な状態からスタートしました。モデルの変更に伴うコードの修正や、精度向上のためのハイパーパラメータの調整など、各種技術について公式ドキュメントやAIを活用し、1行ごとの意味の理解を試みながら制作を進めました。実務未経験のため、自身の現在の技術力を客観的に評価することは難しいですが、エラーに直面するたびに模索しながら解決するプロセスを学びました。今後はより多様なプロジェクトに挑戦し、さらなる開発効率の向上と技術の深掘りに努めたいと考えています。
...
8 directories, 536files(notsakura/sakuraの画像をのぞいて12files)
