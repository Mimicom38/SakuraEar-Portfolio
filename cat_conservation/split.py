...
from sklearn.model_selection import train_test_split #自動シャッフル＆サイズ指定で分割
...
def split_and_copy(src_label):
  ...
  os.makedirs(val_dir, exist_ok=True) #あった場合はエラーを回避

  all_images = [f for f in os.listdir(src_dir) if f.endswith((".jpg", ".png"))]
  train_imgs, val_imgs = train_test_split(all_images, test_size=0.2, random_state=42)#random_stateは同じ結果が出るように固定するためのシード値。42に意味はない(SF小説由来)
#train:val = 8:2
